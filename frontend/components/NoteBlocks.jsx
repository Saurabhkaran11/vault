...
export const emptyBlock = (kind) => {
  ...
  if (kind === "image") return { id: uid(), kind, src: "", name: "" };
  return { id: uid(), kind, text: "", indent: 0 };
};

const SLASH_OPTIONS = [
  ["todo", "☑ To-do"], ["bullet", "• Bullet"], ["heading", "H Heading"],
  ["table", "▦ Table"], ["image", "▣ Image"], ["page", "❐ Sub-page"],
];

const INSERT_OPTIONS = [["text", "≡ Text"], ...SLASH_OPTIONS];

/* Insert-anywhere zone: a slim hover strip between two blocks. Click the ＋
   to pick a block type and it lands exactly there — no more append-only
   stacking, no retyping when you remembered something halfway up the page. */
function InsertZone({ index, openMenu, onToggle, onPick }) {
  const open = openMenu === index;
  return (
    <div className={`izone ${open ? "open" : ""}`}>
      <button className="izone-btn" title="Insert a block right here" aria-label="Insert a block here"
        onClick={(e) => { e.stopPropagation(); onToggle(open ? null : index); }}>＋</button>
      <div className="izone-line" aria-hidden="true" />
      {open && (
        <div className="slashmenu izone-menu" role="menu" onClick={(e) => e.stopPropagation()}>
          {INSERT_OPTIONS.map(([k, label]) => (
            <button key={k} type="button" onClick={() => onPick(index, k)}>{label}</button>
          ))}
        </div>
      )}
    </div>
...
export default function NoteBlocks({ blocks = [], onChange }) {
  ...
  const [zoneMenu, setZoneMenu] = useState(null); // index of the open insert-zone menu
  const refs = useRef({});
  const set = (next) => onChange(next);

  /* any outside click closes an open insert menu */
  useEffect(() => {
    if (zoneMenu === null) return;
    const close = () => setZoneMenu(null);
    window.addEventListener("click", close);
    return () => window.removeEventListener("click", close);
  }, [zoneMenu]);

  /* insert a block of any kind at an exact position */
  ...
  /* insert a new block immediately AFTER the current one —
  ...
  const convert = (b, kind) => {
    ...
    else if (kind === "image") upd(b.id, { kind, src: "", name: "", text: undefined });
    else if (kind === "todo") upd(b.id, { kind, text: "", done: false });
    else upd(b.id, { kind, text: "" });
  };

  /* image block: read the chosen file as a dataURL (1.5 MB cap) */
  const onImage = (b, f) => {
    if (!f) return;
    if (f.size > 1.5 * 1024 * 1024) {
      alert(`“${f.name}” is ${(f.size / 1048576).toFixed(1)} MB — max is 1.5 MB for inline images.`);
      return;
    }
    const r = new FileReader();
    r.onload = () => upd(b.id, { src: r.result, name: f.name });
    r.readAsDataURL(f);
  };
  ...
  return (
    <div className="blocks">
      {blocks.map((b, bi) => (
        ...
        <InsertZone index={bi} openMenu={zoneMenu} onToggle={setZoneMenu} onPick={insertAt} />
        <div className={`block ${dragId === b.id ? "dragging" : ""}`}
          ...
          {b.kind === "image" && (
            <div className="bimage">
              {b.src ? (
                <>
                  <img className="bimg" src={b.src} alt={b.name || "Embedded image"} />
                  {b.name && <div className="bcount" style={{ marginTop: 4 }}>▣ {b.name}</div>}
                </>
              ) : (
                <label className="upload-btn" style={{ alignSelf: "flex-start" }}>
                  ▣ Choose an image… (max 1.5 MB)
                  <input type="file" accept="image/*" hidden
                    onChange={(e) => { onImage(b, e.target.files?.[0]); e.target.value = ""; }} />
                </label>
              )}
            </div>
          )}
