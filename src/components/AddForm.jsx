...
import { SECTIONS, GENERIC_TAGS, today, ytId } from "../data/seed.js";
    ...
    <div className="form">
      ...
      <div className={`dropzone full ${dragOver ? "over" : ""}`} role="button" tabIndex={0}
        ...
        <input ref={fileRef} type="file" hidden onChange={(e) => { handleFiles(e.target.files); e.target.value = ""; }}
          accept=".pdf,.doc,.docx,.txt,.md,.epub,.csv,.png,.jpg,.jpeg,.webp" />
      </div>
      {file && (
      ...
      <div className="full">
        <div className="hint" style={{ marginBottom: 6 }}>Tags — type your own (Enter to create) or tap a suggestion:</div>
